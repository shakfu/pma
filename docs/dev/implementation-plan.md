...
Built: 6.1 to 6.10 and 6.12. A pass runs every node, a rule's for nothing and an agent's behind `--yes`, `max_parallel` at a time and bounded by `batch_budget`; `edit` goes through `dispatch::prepare` and the phase 1 gates and leaves a run to review; calls are flattened at propose time; targets are `dispatch`'s and arguments come through `--set`, recorded on the instance; an instance is frozen at the revision it started under and its caps are enforced where units and runs are written.

Not built: 6.11, and the iteration half of 6.9 and 6.10. `retry`, a lap edge and a self-edge parse, bound and cost, and the runtime takes none of them -- nothing increments a unit's lap, and the frontier cannot tell a unit a node routed onward from one an edge routed back in. That is one change in `pass.rs`, not three: a back edge has to mint a unit rather than pass one. Also outstanding from 6.7: `stop`, and `pma report --by node` and `--by workflow`; from 6.3, a reason per drop taken from the model rather than recorded against the move; and a node's `publish`, which is parsed and ignored, since an agent node's worktree is discarded after the run.
